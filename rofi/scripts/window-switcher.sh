#!/bin/bash
selected=$(hyprctl clients -j | jq -r '.[] | select(.workspace.id != 1) | select(.title != "ピクチャーインピクチャー") | "\(.title) (\(.address))\u0000icon\u001f\(.class)"' | rofi -dmenu -show-icons -theme ~/.config/rofi/themes/main.rasi)
